...
<style>
    ...
    .cyu-options-table {
        /* Without this, the auto table-layout algorithm sizes the "Option Text" column (and
           the box inside it) to fit its content instead of respecting the other columns'
           width="..." attributes - long option text just kept widening the box/column
           sideways rather than wrapping inside it. */
        table-layout: fixed;
    }
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex justify-content-center gap-1">
                                                        <?php
                                                        // A wrong option may be deleted whenever another option remains. A correct
                                                        // option still needs another correct option to remain after deletion.
                                                        ...
                                                        <?php } elseif ($isCorrect && $remainingCorrectIfDeleted <= 0) { ?>
                                                            <button type="button" class="btn btn-outline-danger waves-effect waves-light rounded-circle cyu-action-btn" title="A question must have at least one correct option - mark another option correct before deleting this one" disabled><i class="mdi mdi-trash-can-outline"></i></button>
                                                        <?php } else { ?>
                                                            <button type="button" class="btn btn-outline-danger waves-effect waves-light rounded-circle cyu-action-btn" title="Delete"
                                                                onclick="quizDeleteOption(this, '<?php echo $eachoptiondata['o_id'] ?>')"><i class="mdi mdi-trash-can-outline"></i></button>
                                                        <?php } ?>
                                                    </div>
                                                </td>
                                            </tr>
...
<script>
    function quizDeleteOption(button, optionId) {
        ...
        fetch('<?php echo base_url('Assessment/trainings/updatedateformat') ?>', {
                ...
                body: new URLSearchParams({
                    ...
                    column: 'status',
                    id: optionId,
                    scourse_id: '<?php echo $row['scourse_id'] ?>',
                    question_id: '<?php echo $row['q_id'] ?>',
    ...
    function updateDate(element, column, id) {
        ...
        fetch('<?php echo base_url('Assessment/trainings/updatedateformat') ?>', {
            ...
            .catch(function() {
                // No reload happens here (a network-level failure means there's nothing new to
                // show), so this alert is the only feedback the user gets - previously this was
                // a console.log only, which nobody watching the page would ever see, making the
