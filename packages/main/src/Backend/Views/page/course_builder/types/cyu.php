<?php if ((in_array('46', $arrayuserlevel) || in_array('5', $arrayuserlevel) || in_array('44', $arrayuserlevel))) { ?>
    ...
    <style>
        ...
        .cyu-options-table {
            /* Without this, the auto table-layout algorithm sizes the "Option Text" column
               (and the box inside it) to fit its content instead of respecting the other
               columns' width="..." attributes - long option text just kept widening the
               box/column sideways rather than wrapping inside it. */
            table-layout: fixed;
        }
                ...
                <div class="modal-body">
                    ...
                    ?>
                            <form action="<?php echo base_url('Assessment/trainings/setting_data_update') ?>" method="POST"><?= csrf_field() ?>
                                ...
                                <?php if ($x == '59') { ?>
                                    <label><b><?php echo lang('UI_Text.CB_Default_SCQ'); ?></b> <?php echo $assessment_scqmcq_sets[$x] ?></label>
                                <?php } elseif ($x == '60') { ?>
                                    <label><b><?php echo lang('UI_Text.CB_Default_MCQ'); ?></b> <?php echo $assessment_scqmcq_sets[$x] ?></label>
                                <?php } else { ?>
                                    <label><b><?php echo lang('UI_Text.CB_Default_Colon'); ?></b> <?php echo $assessment_scqmcq_sets[$x] ?></label>
                                <?php } ?>
                                <div class="row">
                                    <div class="col-lg-10">
                            ...
                            <tbody id="table-body">
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td>
                                                <textarea class="cyu-option-text" rows="2"
                                                    onBlur="updateDate(this,'values','<?php echo $eachoptiondata['o_id'] ?>')"><?php echo htmlspecialchars($eachoptiondata['values']) ?></textarea>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-1">
                                                    <?php
                                                    // Must mirror updateoptioneditableformat()'s server-side rule: refuse a delete
                                                    // that would leave zero correct options among what remains. For SCQ (exactly
                                                    // one correct answer) this always blocks the correct one; for MCQ, deleting one
                                                    // of several correct answers is fine as long as another correct one remains.
                                                    // Wrong options are never blocked by this rule. Matches the same rule applied
                                                    // to the Quiz (SCQ/MCQ) option editor in option_add_view.php.
                                                    ...
                                                    <?php } else { ?>
                                                        <button type="button" class="btn btn-outline-danger waves-effect waves-light rounded-circle cyu-action-btn" title="<?php echo lang('Buttons.Delete'); ?>" aria-label="<?php echo lang('Buttons.Delete'); ?>"
                                                            onclick="cyuDeleteOption(this, '<?php echo $eachoptiondata['o_id'] ?>')"><i class="mdi mdi-trash-can-outline" aria-hidden="true"></i></button>
                                                    <?php } ?>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <div class="cyu-feedback-box mb-3">
                                ...
                                <div class="cyu-richbox-wrap">
                                    ...
                                    <div class="cyu-richbox" id="cyuCorrectBox" contenteditable="true"
                                        oninput="cyuOnRichInput(this, 'cyuCorrectHidden')"><?php echo isset($qrow['correct']) ? $qrow['correct'] : ''; ?></div>
                                </div>
                            ...
                            <div class="cyu-feedback-box mb-3">
                                ...
                                <div class="cyu-richbox-wrap">
                                    ...
                                    <div class="cyu-richbox" id="cyuIncorrect2Box" contenteditable="true"
                                        oninput="cyuOnRichInput(this, 'cyuIncorrect2Hidden')"><?php echo isset($qrow['incorrect2']) ? $qrow['incorrect2'] : ''; ?></div>
                                </div>
                            ...
                            <div class="cyu-feedback-box mb-3">
                                ...
                                <div class="cyu-richbox-wrap">
                                    ...
                                    <div class="cyu-richbox" id="cyuIncorrectBox" contenteditable="true"
                                        oninput="cyuOnRichInput(this, 'cyuIncorrectHidden')"><?php echo isset($qrow['incorrect']) ? $qrow['incorrect'] : ''; ?></div>
                                </div>
...
<script>
    ...
    function switchCorrectAnswer(oldOptionId, newOptionId) {
        ...
        // X-Requested-With marks this as AJAX so CodeIgniter's Debug Toolbar doesn't append
        // its loader <script> after the JSON body, which otherwise breaks JSON.parse() on
        // every single response (see updateDate() below).
        fetch(url, {
                method: 'POST',
    ...
    // Keep deletion separate from the generic field-save path. A delete must not look
    // successful until the server has explicitly accepted it; on success remove the row
    // immediately, then refresh the pane to recalculate numbering and delete-button states.
    function cyuDeleteOption(button, optionId) {
        if (!confirm(<?php echo json_encode(lang('Alert.Aler_002')); ?>)) return;
        ...
        fetch('<?php echo base_url('Assessment/trainings/updatedateformat') ?>', {
                ...
                body: new URLSearchParams({
                    ...
                    column: 'status',
                    id: optionId,
                    scourse_id: '<?php echo $scourse_id ?>',
                    question_id: '<?php echo isset($qrow['q_id']) ? $qrow['q_id'] : '' ?>',
            ...
            .then(function(text) {
                ...
                if (!result || result.status !== 'OK') {
                    ...
                    alert(result && result.status ? result.status : 'The option could not be deleted.');
                    return;
                }
            ...
            .catch(function(error) {
                button.disabled = false;
                console.log('Option delete failed:', error);
                alert('The option could not be deleted. Please try again.');
            });
    }

    function updateDate(element, column, id) {
        // A <textarea>'s .innerText/.textContent only ever reflects its ORIGINAL markup
        // content, not what the user is currently typing - only .value tracks live edits.
        // The option-score box is still a contenteditable <div>, which has no .value, so it
        // still needs .innerText.
        var value;
        ...
        // X-Requested-With marks this as AJAX so CodeIgniter's Debug Toolbar doesn't append
        // its loader <script> after the JSON body - without it, the response was e.g.
        // '{"status":"OK"}<script id="debugbar_loader" ...>...', which JSON.parse() always
        // rejects (trailing content after valid JSON is a syntax error), so the real
        // success/failure status was never actually readable - every response looked like a
        // parse failure, regardless of what the server actually said.
        fetch('<?php echo base_url('Assessment/trainings/updatedateformat') ?>', {
                method: 'POST',
                body: new URLSearchParams({
                    value: value,
            ...
            .then(function(text) {
                ...
                try {
                    ...
                    alert('Something unexpected happened while saving - reloading to show the current state.');
                }
            ...
            .catch(function() {
                ...
                // so the indicator needs to be hidden explicitly here. No reload happens either
                // (a network-level failure means there's nothing new to show), so this alert is
                // the only feedback the user gets - previously this was a console.log only,
                // which nobody watching the page would ever see, making the click look like it
    ...
    function cyuOnRichInput(box, hiddenId) {
