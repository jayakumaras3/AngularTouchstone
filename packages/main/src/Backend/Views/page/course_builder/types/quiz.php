                            ...
                            <div class="d-flex align-items-start gap-3 mb-4">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="quiz-textarea-wrap">
                                        <textarea class="form-control" id="quizNewQuestionText" name="question" maxlength="1000" placeholder="<?php echo lang('UI_Text.CB_Question_Placeholder'); ?>" required></textarea>
                                    </div>
                                </div>
                            </div>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <form class="form-horizontal"
                                            ...
                                            method="POST"
                                            onsubmit="return quizDeleteQuestion(event, this)"><?= csrf_field() ?>
                                            <input type="hidden" name="question_id" value="<?php echo $eachQuestion['q_id']; ?>">
                                            <button type="submit"
...
<script>
    // Delete in place only after the server explicitly confirms success. Refreshing the
    // builder pane afterwards keeps numbering, counts and quiz settings in sync.
    function quizDeleteQuestion(event, form) {
        event.preventDefault();
        if (!confirm(<?php echo json_encode(lang('Alert.Aler_002')); ?>)) return false;
        ...
        fetch(form.action, {
            ...
            .then(function(result) {
                if (!result || result.status !== 'OK') {
                    ...
                    alert(result && result.message ? result.message : 'The question could not be deleted.');
                    return;
                }
            ...
            .catch(function(error) {
                ...
                alert('The question could not be deleted. Please try again.');
            });
