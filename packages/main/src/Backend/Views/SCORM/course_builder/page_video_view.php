                ...
                if ($return_page == 2) { ?>
                    ...
                    <?php } elseif ($row['type'] == 10 || $row['type'] == 11 || $row['type'] == 12) {
                        ...
                        <div class="text-page-container" style="padding: 30px; overflow-y: auto; height: 86vh;">
                            <?php
                            // This page only ever loads Bootstrap 3.3.7 (see header.php) or the
                            // course's own export-theme bootstrap.css, neither of which defines the
                            // Bootstrap 4/5 utility classes "img-fluid"/"rounded" used here before -
                            // so the image had no max-width constraint at all and rendered at its
                            // native size, overflowing across into (and covering) the text side.
                            // Explicit inline sizing and a plain flex layout (instead of .row/.col-*,
                            // which depend on that same possibly-incomplete grid CSS) make this
                            // self-contained regardless of which theme CSS is actually active.
                            $textImageStyle = 'display:flex; align-items:flex-start; gap:24px; flex-wrap:wrap;';
                            $textImageColStyle = 'flex:1 1 300px; min-width:0;';
                            $textImageImgStyle = 'max-width:100%; height:auto; border-radius:6px; display:block;';
                            ?>
                            <?php if ($row['type'] == 11 && $hasPageImage) { ?>
                                <div style="<?php echo $textImageStyle; ?>">
                                    <div style="<?php echo $textImageColStyle; ?>"><img src="<?php echo $pageImageUrl; ?>" alt="<?php echo esc($imageAlt); ?>" style="<?php echo $textImageImgStyle; ?>"></div>
                                    <div style="<?php echo $textImageColStyle; ?>"><?php echo $row['content'] ?? ''; ?></div>
                                </div>
